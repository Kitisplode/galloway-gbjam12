...
if (surface_exists(surface_id_1) && surface_exists(surface_id_2))
{
	...
	for (var _i = 0; _i < ds_list_size(list_lights); _i++)
	{
		...
		var _temp_x = _temp_id.x - (sprite_get_xoffset(_temp_id.sprite_index) + _temp_id.sprite_width / 2 * -_temp_id.image_xscale);
		var _temp_y = _temp_id.y - sprite_get_yoffset(_temp_id.sprite_index) + _temp_id.sprite_height / 2;
		if (_temp_value._light > 0)
		{
			...
			draw_circle_color(_temp_x-1, _temp_y - _temp_id.z,
							_temp_value._light * 2 + abs(round(cos(degtorad(light_cycle)) * 3)),
							c_white,c_white, 0);
			surface_reset_target();
